...
height = 500
entered_text = ""
password_dictornary = {"example": ["example_username", "example_password_1234"]}
...
class PasswordManager(tk.Tk):
    ...
    def show_frame(self, name):
        ...
        frame.place(width=width, height=height, relx=0.5, rely=0.5, anchor="center")
        frame.tkraise()
...
class Startmenu(tk.Frame):
    def __init__(self, root):
        ...
        # todo: making a cleaner startscreen (only the two buttons, bigger buttons)
...
class Dictonary(tk.Frame):
    def __init__(self, root):
        ...
        self.option_list = ["You have no passwords saved"]
        ...
        tk.Label(self, text="Your password:", bg=bg_color, fg=fg_color).grid(row=3, column=2, sticky="e")
        ...
        tkm.Button(self, text="Remove", command=lambda: self.remove_pass(), bg=bg_button_passive,
                   fg=fg_button_passive, activebackground=bg_button_active, borderless=True,
                   overbackground=bg_button_hover).grid(row=3, column=0, sticky="s")
        # Copy Buttons
        self.copy1 = tkm.Button(self, text="Copy", command=lambda: [copy_to_clipboard(self, self.name_website),
                                                                    self.refresh_copy_button(self.copy1)],
                                bg=bg_button_passive,
                                fg=fg_button_passive, activebackground=bg_button_active, borderless=True,
                                overbackground=bg_button_hover)
        self.copy2 = tkm.Button(self, text="Copy", command=lambda: [copy_to_clipboard(self, self.name_password),
                                                                    self.refresh_copy_button(self.copy2)],
                                bg=bg_button_passive,
                                fg=fg_button_passive, activebackground=bg_button_active, borderless=True,
                                overbackground=bg_button_hover)
        exit_button(self)
        ...
        self.name_password.grid(row=3, column=3, sticky="w")

    def callback(self, *args):
        self.add_option()
        self.name_website.config(text=self.variable.get())
        self.name_password.config(text=password_dictornary[self.variable.get()])
    ...
    def refresh_copy_button(self, button, time=1500, text1="Copy", text2="Copied"):
        def change_text():
...
class Generator(tk.Frame):
    def __init__(self, root):
        ...
        self.variable = tk.StringVar(self)
        self.variable.set(self.length_list[9])
        self.get_length = 10

        # changing bg of the frame
        self.config(bg=bg_color)

        # Widgets to show website
        ...
        self.length.grid(row=1, column=3, sticky="w")
        self.variable.trace("w", self.callback)

        # Widgets to show/generate passwords
        tk.Label(self, text="Your password: ", bg=bg_color, fg=fg_color).grid(row=1, sticky="w")
        var1 = tk.OptionMenu(self, self.variable, *self.length_list)

        tk.Label(self, text="Length:", bg=bg_color, fg=fg_color).grid(row=1, column=2)
        ...
        tkm.Button(self, text="Generate new password",
                   ...
                   bg=bg_button_passive,
                   fg=fg_button_passive, activebackground=bg_button_active, borderless=True,
                   overbackground=bg_button_hover).grid(row=2,
                                                        sticky="E")
        tkm.Button(self, text="Submit", command=lambda: self.submit(), bg=bg_button_passive,
                   fg=fg_button_passive, activebackground=bg_button_active, borderless=True,
                   overbackground=bg_button_hover).grid(row=2, column=1, )
    ...
    def submit(self):
        global password_dictornary
        pg.save_pass(password_dictornary, self.website.get(), self.pass_text.get(), "")
        n = 0
        self.website.delete(0, "end")
        for name in password_dictornary:
            n += 1
            print(f"Website_{n}: {name}\nPassword: {password_dictornary[name]}\n")
...
if __name__ == '__main__':
    ...
    App.mainloop()
    # backup written: 10/8/2021 8:27pm
